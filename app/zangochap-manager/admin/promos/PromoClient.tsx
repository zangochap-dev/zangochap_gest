...
function NewPromoModal({ promo, products, categories, user, onClose }: { promo?: any; products: any[]; categories: any[]; user: any; onClose: () => void }) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    // Always include selected products
    const selected = products.filter(p => selectedProductIds.includes(p.id));
    const selectedSet = new Set(selectedProductIds);

    // Filter non-selected products
    let pool = products.filter(p => !selectedSet.has(p.id));

    // Category filter
    if (selectedSearchCategory !== 'all') {
      pool = pool.filter(p => p.categoryId === selectedSearchCategory);
    }
    ...
    if (query) {
      ...
      // Limit to 6 items to avoid huge lists
      pool = pool.slice(0, 6);
    }

    return [...selected, ...pool];
  }, [products, selectedProductIds, productSearch, selectedSearchCategory]);
    ...
    startTransition(async () => {
      try {
        const payload = {
          ...
          isAutomatic,
          type,
          value: type === 'GIFT' ? 0 : value,
          giftProductId: type === 'GIFT' ? giftProductId : null,
          rule,
          minAmount,
          minQuantity,
          maxGlobalUses,
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  ...
                  <select className="field-input" value={type} onChange={e => setType(e.target.value)}>
                    ...
                    <option value="GIFT">Produit cadeau (Offert) 🎁</option>
                  </select>
                </div>
                {type === 'GIFT' ? (
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[11px] font-bold uppercase text-[#888]">Produit cadeau offert *</label>
                    <select className="field-input text-xs" value={giftProductId} onChange={e => setGiftProductId(e.target.value)}>
            ...
            <div className="p-6 bg-[#FAFAF9] rounded-xl border border-[#F1F1EF] space-y-5">
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {/* Recherche avancée pour produits publiés */}
                  <div className="flex gap-2">
                    ...
                    <select
                      ...
                      style={{ padding: '8px 12px', width: '130px', height: '36px' }}
                      value={selectedSearchCategory}
                      onChange={e => setSelectedSearchCategory(e.target.value)}
                    >
                      <option value="all">Toutes cats</option>
                      {categories.map(c => (
                  ...
                  <div className="flex flex-col gap-1 max-h-[200px] overflow-y-auto p-2 border border-[#eee] bg-white rounded-lg">
                    {filteredProducts.map(prod => (
                      <button
                        ...
                        className={`flex items-center justify-between p-2 rounded text-left transition-all ${selectedProductIds.includes(prod.id) ? 'bg-orange-50 text-orange-700 font-semibold' : 'hover:bg-gray-50'}`}
                      >
                    ...
                    {filteredProducts.length === 0 && (
                      <p className="text-[11px] text-[#999] text-center py-4">Aucun produit trouvé.</p>
                    )}
                    <p className="text-[10px] text-[#aaa] mt-1 italic">Si aucun produit n'est sélectionné, la remise s'applique à tout le catalogue.</p>
                  </div>
                </div>
              </div>
            </div>
