...
export async function generateUniqueRef(commune?: string, typePrefix?: string) {
  // Find the highest sequence number by sorting by createdAt
  ...
  if (lastOrder && lastOrder.ref.includes('CD-')) {
    const parts = lastOrder.ref.split('-');
  ...
  const basePrefix = typePrefix && typePrefix !== 'Standard' 
    ? `${typePrefix.toUpperCase().replace(/É/g, 'E')}-CD` 
    : 'CD';

  // Ensure uniqueness to prevent P2002 constraint errors
...
export async function createOrder(data: {
  ...
  await decrementStockForOrder(order, session || { name: 'Site Web', email: 'public' });
...
export async function updateOrderStatus(orderId: string, newStatus: string, note?: string) {
  ...
  history.push({
    at: new Date().toISOString(),
    action: `Statut → ${newStatus} par ${session.name}`,
    by: session.email,
    byName: session.name,
  });
  if (note) {
  ...
  if (['CANCELLED', 'RETURNED', 'EXCHANGED'].includes(newStatus.toUpperCase()) && order.stockDecremented) {
...
export async function assignOrderToDeliveryman(orderId: string, deliverymanId: string) {
  ...
  history.push({
    at: new Date().toISOString(),
    action: `Attribuée au livreur : ${driver.name}`,
    by: session.email,
    byName: session.name,
  });
  ...
  revalidatePath("/zangochap-manager/orders");
  revalidatePath("/zangochap-manager/delivery");
  return { success: true };
}
...
async function restoreStockForOrder(order: any, session: any, type: 'RETURN' | 'EXCHANGE' | 'ADJUSTMENT' = 'RETURN') {
  for (const item of order.items) {
    ...
    if (variant) {
      await prisma.productVariant.update({ where: { id: variant.id }, data: { stock: { increment: item.qty } } });
      await recordStockMovement({ variantId: variant.id, type: type as any, quantity: item.qty, orderId: order.id, session });
    }
  }
  const pIds = [...new Set(order.items.filter((i: any) => i.productId).map((i: any) => i.productId))];
  for (const pid of pIds) {
...
export async function duplicateOrder(orderId: string, data: any) {
  ...
  // On construit les notes en incluant le motif de l'échange si présent
  let finalNotes = data.notes || "";
  if (data.type === 'Echange' && data.exchangeReason) {
    finalNotes = `MOTIF ÉCHANGE: ${data.exchangeReason}${finalNotes ? `\n---\n${finalNotes}` : ''}`;
  }

  // On utilise createOrder pour bénéficier de toute la logique de stock/client/ref
