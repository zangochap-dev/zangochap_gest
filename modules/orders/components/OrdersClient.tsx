...
import { Plus, Eye, Package, Trash2, Minus, Search, X, ChevronLeft, ChevronRight, RefreshCw, Copy, Edit3, Maximize, AlertTriangle, Phone, Save, Edit2, Download, MessageCircle, Printer, Truck, Check, ArrowLeftRight, Ban, Users, Calendar, CalendarClock, MoreHorizontal, CreditCard } from "lucide-react";
                  ...
                  return (
                    ...
                    <tr key={order.id}>
                      ...
                      <td>
                        ...
                        <div style={{ fontWeight: 600, color: ['REPROGRAMMED', 'REPRO_DISPO'].includes(order.status) ? 'var(--orange)' : 'var(--ink)' }}>
                        ...
                        {['REPROGRAMMED', 'REPRO_DISPO'].includes(order.status) && (
                          ...
                          <div style={{ fontSize: 9, color: 'var(--orange)', fontWeight: 800, textTransform: 'uppercase' }}>{order.status === 'REPRO_DISPO' ? 'Repro-dispo' : 'Reprogrammée'}</div>
                      ...
                      <td>
                        ...
                        <div className="row-actions">
                          ...
                          <button className="action-btn" title="Envoyer re?u WhatsApp au client" onClick={() => handleWhatsApp(order)} style={{ background: '#dcfce7', color: '#16a34a' }}>
                            <MessageCircle size={14} />
                          </button>
                          <details className="action-menu">
                            <summary className="action-btn" title="Plus d'actions" aria-label="Plus d'actions">
                              <MoreHorizontal size={14} />
                            </summary>
                            <div className="action-menu-panel">
                              {(user?.role === 'admin' || user?.role === 'commercial') && (
                                <button type="button" className="action-menu-item" onClick={() => setOrderToDuplicate(order)}>
                                  <Copy size={14} /> Créer un échange
                                </button>
                              )}
                              {!['DELIVERED', 'CANCELLED', 'REPRO_DISPO'].includes(order.status) && (
                                <button
                                  type="button"
                                  className="action-menu-item"
                                  onClick={() => {
                                    if (confirm('Mettre cette commande en repro-dispo pour demain ?')) handleStatusChange(order.id, 'REPRO_DISPO');
                                  }}
                                >
                                  <CalendarClock size={14} /> Repro-dispo
                                </button>
                              )}
                              <button type="button" className="action-menu-item" onClick={() => setReceiptOrder(order)}>
                                <Printer size={14} /> Imprimer reçu
                              </button>
                              {(user?.role === 'admin' || user?.role === 'commercial') && (
                                <button type="button" className="action-menu-item danger" onClick={() => handleDelete(order.id)}>
                  ...
                  <>
                    ...
                    {!['DELIVERED', 'CANCELLED', 'REPRO_DISPO'].includes(order.status) && (
                      <button
                        className="btn-secondary"
                        onClick={() => {
                          if (confirm('Mettre cette commande en repro-dispo pour demain ?')) onStatusChange(order.id, 'REPRO_DISPO');
                        }}
                        disabled={isPending}
                      >
                        <CalendarClock size={14} /> Repro-dispo
...
function OrderFormModal({ order, mode = 'duplicate', onClose, onConfirm, isPending, onPreviewImage, products = [], onReproDispo }: { order: any; mode?: 'duplicate' | 'edit'; onClose: () => void; onConfirm: (data: any) => void; isPending: boolean; onPreviewImage: (url: string | null) => void; products?: any[]; onReproDispo?: () => void }) {
    ...
    <Modal
      ...
      footer={
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', padding: '4px 0' }}>
          ...
          {mode === 'edit' && onReproDispo && !['DELIVERED', 'CANCELLED', 'REPRO_DISPO'].includes(order.status) && (
            <button
              type="button"
              className="btn-secondary"
